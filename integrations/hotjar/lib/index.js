...
Hotjar.prototype.initialize = function () {
  ...
  function areOptionsValid (options) {
    var validators = {
      isHjidValid: is.number(options.hjid) && !is.nan(options.hjid) && options.hjid !== 0, // Make sure that HJID is a number (and isn't NaN)
      isPlaceholderPolyfillValid: is.bool(options.hjPlaceholderPolyfill) // Make sure we received a boolean.
    }

    for (var validator in validators) {
      if (!validators[validator]) {
        console.warn('Hotjar Integration - ' + validator + ' returned false, not loading.')
        return false
      }
    }
  ...
  (function (h, o, t, j, a, r) {
    ...
    h._hjSettings = {hjid: h._hjSelf.options.hjid, hjsv: 5, hjPlaceholderPolyfill: h._hjSelf.options.hjPlaceholderPolyfill}
