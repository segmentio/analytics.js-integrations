...
var MediaMath = module.exports = integration('MediaMath')
  ...
  .option('events', [])
  .tag('page', '<script src="//pixel.mathtag.com/event/js?mt_id={{ allPagesMtId }}&mt_adid={{ allPagesMtAdId }}&v1=&v2=&v3=&s1=&s2=&s3=">')
  .tag('conversion', '<script src="//pixel.mathtag.com/event/js?{{ query }}">');
...
MediaMath.prototype.track = function(track) {
  ...
  each(events, function(event) {
    ...
    var queryParamPropertyMap = [].concat(event.vParameters || [], event.sParameters || []);
    ...
    var query = foldl(function(query, kv) {
